...
function WelcomePage() {
  ...
  return (
    <main className="min-h-screen bg-background text-foreground py-20 sm:py-28">
      <div className="mx-auto max-w-3xl px-6 text-center">
        <p className="text-[10px] uppercase tracking-[0.38em] text-muted-foreground">Welcome</p>
        <h1 className="mt-5 font-serif text-5xl sm:text-7xl leading-[0.95]">
          You're
          <span className="font-serif-italic block text-muted-foreground mt-2">in.</span>
        </h1>
        ...
        <div className="mt-14 grid gap-5 sm:grid-cols-3 text-left">
          {steps.map((s) => (
            <div key={s.n} className="rounded-2xl bg-card p-7 border border-border">
              <div className="font-serif-italic text-sm text-muted-foreground">{s.n}</div>
        ...
        <Link
          to="/"
          className="mt-14 inline-flex items-center px-8 py-4 rounded-full bg-foreground text-background text-[10px] uppercase tracking-[0.28em] hover:opacity-90 transition"
        >
          Back to home
        </Link>
      </div>
